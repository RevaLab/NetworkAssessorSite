...
function drawPieChart(nodeElement, percentages, getColor, options) {
  var radius = getOptionOrDefault('radius', options)
  var halfRadius = radius / 2
  var halfCircumference = 2 * Math.PI * halfRadius

  if (nodeElement.select('[id^=pie-fill-]').size() <= 0) {
    percentages.forEach((_, idx) => {
      nodeElement
...
export const NodePieBuilder = {
  drawNodePie: function (nodeElement, percentages, getColor, options) {
    ...
    drawPieChart(nodeElement, percentages, getColor, options)
  }
}
